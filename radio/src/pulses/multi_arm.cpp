...
static void sendSetupFrame()
{
  ...
  sendByteMulti('M');
  sendByteMulti('P');
  sendByteMulti(0x80);         // Module Configuration
  sendByteMulti(1);         // 1 byte data
...
static void sendFailFailsafeHeader(uint8_t port)
{
  ...
  sendByteMulti('M');
  sendByteMulti('P');
  sendByteMulti(0x81);         // Failsafe Data
  sendByteMulti(23);         // 22 byte channe + 1 byte mode
  sendByteMulti(g_model.moduleData->failsafeMode);
}
